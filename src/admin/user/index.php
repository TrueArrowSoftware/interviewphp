...
$messages = array();
if (! $permission->CheckOperationPermission("user", "access", $GLOBALS['user']->UserRoleID)) {
    \TAS\Core\Web::Redirect("../index.php");
}
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['delete']) && is_numeric($_GET['delete']) && $permission->CheckOperationPermission("user", "delete", $GLOBALS['user']->UserRoleID)) {
        if (\TAS\Core\DataFormat::DoSecure($_GET['delete']) == 1) {
            $messages[] = array(
                "message" => _("Default user can not be deleted!!!"),
                "level" => 10
            );
        } else {
            ...
            if (User::Delete((int) $_GET['delete'])) {
                ...
                $messages[] = array(
                    "message" => _("Unable to delete user at this moment. Please try again."),
                    "level" => 10
                );
            }
        }
    ...
    if (isset($_GET['status']) && $_GET['status'] == '1') {
        $messages[] = array(
            "message" => _("User status has been updated successfully."),
            "level" => 1
        );
    }
    if (isset($_GET['mode']) && $_GET['mode'] == 'clearfilter') {
        setcookie('admin_user_filter', '', (time() - 25292000));
        \TAS\Core\Web::Redirect("index.php");
    }
}

$pageParse['Content'] .= '<div class="col-md-12 pt-3"> <div class="card card-body card-radius p-0">
<h2 class="borderbottom-set">User Management</h2>';
$pageParse['Content'] .= '<div class="px-3 py-2">' . \TAS\Core\UI::UIMessageDisplay($messages) . '</div>';
$pageParse['Content'] .= '<h6 class="pl-3 py-3"><a href="add.php">Add New User</a> / <a href="index.php?download=true">Download Now (Current filter)</a></h6>';

if (isset($_COOKIE['admin_user_filter']) && $_SERVER['REQUEST_METHOD'] == 'GET') {
    $filterOptions = json_decode($_COOKIE['admin_user_filter'], true);
} else {
    $filterOptions = $_REQUEST;
}

$_COOKIE['admin_user_filter'] = json_encode($filterOptions);
setcookie('admin_user_filter', json_encode($filterOptions), time() + 3600000);
